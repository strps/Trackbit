...
const AuthPage = () => {
        ...
        try {
            if (view === 'signup') {
                await signUp.email({
                    ...
                    onSuccess: () => {
                        // Auto login or ask to verify email depending on config
                        navigate('/dashboard');
                    },
                ...
                await requestPasswordReset({
                    email: formData.email,
                    redirectTo: "/reset-password" // You need a route for this
                }, {
                    onSuccess: () => alert("Check your email for a reset link! (Check server console in dev)"),
                    onError: (ctx) => setError(ctx.error.message)
                });
            }
        } catch (err: any) {
    ...
    const toggleView = (newView: 'signin' | 'signup' | 'forgot') => {
        setView(newView);
