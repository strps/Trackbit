...
export function useHabitLogs() {
    ...
    const selectedHabitId = useTrackerStore(s => s.selectedHabitId);
    const selectedDay = useTrackerStore(s => s.selectedDay);
    ...
    const currentLog = (selectedHabitId && selectedDay)
    ...
    const addExerciseLog = useMutation({
        mutationFn: async (exerciseId: number) => {
            ...
            const session = currentLog?.exerciseSessions[selectedSessionIndex]
            ...
            const res = await fetch(`${API_URL}/exercise-log`, {
                method: 'POST',
                body: JSON.stringify({ sessionId: session.id, exerciseId, sets: [defaultSet] }),
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include'
            });
        ...
        onSuccess: (data, variables, ctx) => {
            ...
            updateCache((newData) => {
                if (!selectedHabitId || !selectedDay) return;
                const session = newData[selectedHabitId]?.dayLogs[selectedDay].exerciseSessions[selectedSessionIndex!];
                const ex = session?.exercises.find(e => e.tempId === ctx?.tempId);
                if (ex) {
                    ex.id = data.id;
                    delete ex.tempId;
                }
    ...
    const saveSet = useMutation({
        mutationFn: async (payload: { exerciseLog: ExerciseLogEntry, set: ExerciseSet }) => {
            ...
            const res = await fetch(`${API_URL}/exercise-set`, {
                ...
                body: JSON.stringify({
                    ...
                    reps: payload.set.reps,
                    weight: payload.set.weight
                }),
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include'
            });
        ...
        onMutate: async ({ exerciseLog, set }) => {
            if (!selectedHabitId || !selectedDay) return;
            const tempId = `set-${Math.random()}`;
            ...
            updateCache((newData) => {
                ...
                if (exerciseLog) {
                    if (set?.id) {
                        // Update
                        const existing = exerciseLog.exerciseSets.find(s => s.id === set.id);
                        if (existing) {
                        ...
                        exerciseLog.exerciseSets.push({ ...set, tempId });
                    }
                }
            });
            return { tempId };
        },
        onSuccess: (data, { exerciseLogId, set }, ctx) => {
            if (!set.id) { // If it was a create TODO: this should be updates, as the estrucutre is diferenct: sesions.logs.sets
                updateCache((newData) => {
                    if (!selectedHabitId || !selectedDay) return;
                    const log = newData[selectedHabitId]?.dayLogs[selectedDay];
                    const exercise = log?.exercise.find(e => e.id === exerciseLogId);
                    const optimisticSet = exercise?.sets.find(s => s.tempId === ctx?.tempId);
                    if (optimisticSet) {
                        optimisticSet.id = data.id;
                        delete optimisticSet.tempId;
                    }
                });
            }
        }
    ...
    const deleteSet = useMutation({
        mutationFn: async (setId: number) => {
            await fetch(`${API_URL}/set/${setId}`, { method: 'DELETE' });
        },
